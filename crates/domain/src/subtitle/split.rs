...
use super::transcript::{Cue, CueFlags, FieldOrigin, RangeUsize, TimelineSource, Transcript, Word};
...
impl RuleSplitConfig {
    ...
    pub fn detect_cjk_from_words(words: &[Word]) -> bool {
        let mut cjk = 0usize;
...
pub fn rule_split(transcript: &Transcript, cfg: &RuleSplitConfig) -> DomainResult<Transcript> {
    if transcript.timeline_source != TimelineSource::AsrWords {
        return Err(DomainError::InvalidArgument(
            "rule_split requires AsrWords timeline".into(),
        ));
    }
...
#[cfg(test)]
mod tests {
    use super::super::transcript::{EngineFingerprint, PROB_UNAVAILABLE};
    use super::*;
